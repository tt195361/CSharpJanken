...
namespace CSharpJanken
{
    ...
    internal class JankenApp
    {
        ...
        internal static int JudgeResult(int yourHand, int computerHand)
        {
            ...
            {
                // あなたがコンピュータに勝つ場合は、
                //   - あなたが GU (0) で、コンピュータが CYOKI (1)、
                //   - あなたが CYOKI (1) で、コンピュータが PA (2)、
                //   - あなたが PA (2) で、コンピュータが GU (0)
                // の 3 通りです。これらの組み合わせは、あなたの手の次の手が
                // コンピュータの手になっていると考えることができます。
                // GU (0) の次は CYOKI (1)、CYOKI (1) の次は PA (2) という
                // ぐあいです。PA (2) 場合、その次は最初に戻って GU (0) になると
                // 考えればうまくゆきます。プログラムの実装ですが、
                // "その次" は "+ 1" で、"最初に戻る" は割り算の余りを求める
                // "% HAND_MAX" を使います。HAND_MAX の値は、手の最大値 2 に
                // 1 を加えた 3 にしています。あなたの手が PA (2) の場合、
                // "+ 1" の結果は 3、これを 3 で割ると余りは 0 で、結果は
                // GU (0) になります。 "(あなたの手 + 1) % HAND_MAX" が
                // コンピュータの手と等しいなら、あなたの勝ちと判断できます。
                result = YouWin;
            }
            else
            {
